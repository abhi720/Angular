  ...
  chooseSeat(seat: any){
    if(seat.seatStatus){
      ...
      if(seatid > -1){
        ...
        console.log(this.passengerForm.get('pPName').value);
        if(this.seatsChoose.length == 1){
          this.passengerArray.push(this.fb.group({
            Gender:'',
            Name:this.passengerForm.get('pPName').value,
            Age:'',
            Seat:seat.seatNo
          }));
          this.passengerData.push({Name: this.passengerForm.get('pPName').value ,Age: "", Gender: "", SeatNo: seat.seatNo.toString()});
          this.datasource = new MatTableDataSource(this.passengerData);
        }
        else{
          this.passengerArray.push(this.fb.group({
